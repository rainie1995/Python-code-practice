...
# 数据加载
df = pd.read_csv(r'D:\python_code\sample\time-series-analysis\bitcoin-master\bitcoin_2012-01-01_to_2018-10-31.csv')
# 将时间作为 df 的索引
df.Timestamp = pd.to_datetime(df.Timestamp) # 转化为时间格式
...
# 正确显示中文标签
plt.rcParams['font.sans-serif']=['SimHei'] # 用来配置文件来自定义图形的各种默认属性
...
df_month2 = pd.concat([df_month2, future]) # 数据合并
