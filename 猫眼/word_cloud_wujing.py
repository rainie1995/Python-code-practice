...
if __name__ == '__main__':
	...
	data = pd.read_sql('select comment from moviecomment where ranking = 2;', conn)
	comment = jieba.cut(str(data),cut_all=False) # 对data["content"]内容进行分词，不采用全模式 
	...
	plt.axis("off") # 不显示坐标轴
	wc.to_file(r'D:\python_code\sample\猫眼\word_cloud_lowranking.png') # 保存图片
	conn.commit()
	conn.close()